...
class PoissonModel {
    ...
    predict(lambdas, homeStats, awayStats, projectedHomeGoals, projectedAwayGoals) {
        ...
        homeWinProb *= (1 + (homeFormFactor - awayFormFactor) * 0.2) * disparityBoost; // Réduit de 0.3 à 0.2 pour moins de surconfiance
        awayWinProb *= (1 + (awayFormFactor - homeFormFactor) * 0.2) * disparityBoost; // Idem
        ...
        // Refined adjustments based on backtest (replace ad-hoc with these)
        // BTTS: Boost high (from PDF: 70-79% actual 82% > predicted)
        if (markets['btts'] > 70) markets['btts'] *= 1.08;  // Increased from 1.05
        if (markets['btts'] < 50) markets['btts'] *= 0.95;  // Slight reduce low for over-confidence
        markets['btts'] = Math.min(100, Math.max(0, markets['btts']));
        markets['btts_no'] = 100 - markets['btts'];

        // Match over/under 1.5, 2.5, under 3.5: Targeted boosts
        const ou_markets = ['match_over_1.5', 'match_over_2.5', 'match_under_3.5'];
        for (const key of ou_markets) {
            if (markets[key] > 70) markets[key] *= 1.07;  // From PDF: under-predict success in high tranches
            if (markets[key] < 60 && key.includes('over')) markets[key] *= 0.92;  // Reduce over-confidence in low over
            if (markets[key] > 80 && key === 'match_under_3.5') markets[key] *= 1.05;  // Specific for under 3.5 (PDF shows 85% predicted vs. 94% actual in 80-89%)
            markets[key] = Math.min(100, Math.max(0, markets[key]));
        }

        // Favorites: Reduce more in low tranches (PDF: 51% actual in 0-59%)
        if (markets['favorite_win'] < 60) markets['favorite_win'] *= 0.92;  // From 0.85 global
        if (markets['favorite_win'] > 80) markets['favorite_win'] *= 0.98;  // Slight reduce high to avoid over
        markets['favorite_win'] = Math.min(100, markets['favorite_win']);

        // Apply simple Platt-like calibration (hardcoded from PDF averages)
        // E.g., for BTTS: a=1.1, b=0.05 (boost overall)
        const calibrate = (p, a=1.0, b=0.0) => 1 / (1 + Math.exp(-(a * Math.log(p / (1 - p)) + b)));
        markets['btts'] = calibrate(markets['btts'] / 100, 1.1, 0.05) * 100;
        // Add similar for other markets if needed, e.g., for match_over_2.5
        ...
        // For other over/under, similar calibration can be added based on PDF data
...
async function runBacktestAnalyzer() {
        ...
        try {
            ...
            for (const fixture of finishedMatches) {
                ...
                let confidenceAdjustment = isEarlySeason ? {
                    ...
                    'btts': (matchesPlayed / 6) * 0.7 + 0.3, // Boosté pour BTTS en début
                    'result': (matchesPlayed / 6) * 0.5 + 0.5 // Ajusté pour result/double chance
    ...
    try {
        ...
        // Calcul des taux et intervalles de confiance
        for (const market in trancheAnalysis) {
            for (const key in trancheAnalysis[market]) {
                ...
                if (tranche.total > 0) {
                    ...
                    console.log(chalk.blue(`   -> ${market} (${key}): ${tranche.success}/${tranche.total} = ${tranche.rate.toFixed(2)}% (CI: ${tranche.ci.lower.toFixed(2)}-${tranche.ci.upper.toFixed(2)}%)`));
                }
            }
        }
        ...
        for (const key in globalTrancheSummary) {
            ...
            if (tranche.total > 0) {
                ...
                tranche.rate = (tranche.success / tranche.total) * 100;
                tranche.ci = calculateConfidenceInterval(tranche.success, tranche.total);
                console.log(chalk.magenta(`   -> Global (${key}): ${tranche.success}/${tranche.total} = ${tranche.rate.toFixed(2)}% (CI: ${tranche.ci.lower.toFixed(2)}-${tranche.ci.upper.toFixed(2)}%)`));
            }
        }
